...
// Same as pushFileToGitHub, but for binary files (hero images) where the
// caller already has base64-encoded bytes and must NOT have them re-encoded
// as if they were a UTF-8 string (that would corrupt the image).
export async function pushBinaryFileToGitHub(filePath, base64Content, commitMessage = "AI update", repo, owner) {
  ...
  if (!process.env.GITHUB_TOKEN || !targetOwner || !targetRepo) {
    return { ok: false, error: "GITHUB_TOKEN / GITHUB_REPO_OWNER / GITHUB_REPO_NAME are not configured on the server." };
  }

  try {
    let sha = null;
    try {
      const { data } = await getOctokit().rest.repos.getContent({ owner: targetOwner, repo: targetRepo, path: filePath });
      sha = data.sha;
    } catch (e) {
    ...
    return { ok: true, message: `File ${filePath} pushed to ${targetOwner}/${targetRepo}.` };
  } catch (error) {
